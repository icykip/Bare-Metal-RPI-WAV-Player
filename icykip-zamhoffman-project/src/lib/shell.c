...
static unsigned int commands_end = 7;

unsigned int add_cmd(char * name, char * descript, int (*f)(int, const char *)){
	if(commands_end == commands_space) shell_printf("commands full");
	else{
	    commands[commands_end] = {name, descript, f};
	    commands_end++;
	}
	return 1;
} 

unsigned int rmv_cmd(char * name){
    if(commands_end != 0){
	for(int i = 0; i < commands_end; i++){
	...
	shell_printf("command does not exist");
	return 0;
    }
    shell_printf("there are no commands");
}
